...
var Anchorman = function(opts) {
  ...
  var conf = {
    ...
    createRedisClient: function() {
      throw new Error('node-anchorman was not given any redis clients');
    },
  ...
  self.redis = self.conf.redis || self.conf.createRedisClient();

  self.subRedis = self.conf.subscriberRedis || self.conf.createRedisClient();
